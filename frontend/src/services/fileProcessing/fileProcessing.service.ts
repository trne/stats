  ...
  return data
    .map(dev => {
      ...
      return {
        ...
        score: 
          // Calculate ratio of additions to deletions (avoiding division by zero)
          dev.commits.totals.d === 0 
            ? dev.commits.totals.a * 1000  // If no deletions, just use additions
            : (dev.commits.totals.a / dev.commits.totals.d) * 1000 +  // Higher ratio = better score
          // Add absolute number of additions as a tiebreaker
          dev.commits.totals.a
      };
    })
